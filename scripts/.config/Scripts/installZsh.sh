...
if ! command -v git &>/dev/null; then
  ...
  case $yn in
  [yY][eE][sS] | [yY])
    echo $GREEN"Installing git"$RESTORE
    sleep 2
    sudo $packageManager git
    ;;
  [nN][oO] | [nN])
    echo $RED"Aborting..."$RESTORE
    exit 1
    ;;
  *) exit ;;
  esac
...
case $yn in
[yY][eE][sS] | [yY])
  sudo $packageManager fastfetch
  sed -i "1i fastfetch" ~/.zshrc
  ;;
[nN][oO] | [nN]) echo $RED"Skipping Fastfetch installation..."$RESTORE ;;
*) exit ;;
esac
